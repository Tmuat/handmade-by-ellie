        ...
        if order_form.is_valid():
            ...
            for product_sku, quantity in bag.items():
                try:
                    ...
                    if quantity > product_stock.available_stock:
                        ...
                        product_stock.available_stock = product_stock.available_stock - quantity
                        product_stock.save()
            ...
            if discount_included:
                if discount.active:
                        ...
                        if timezone.now() < discount.expiry:
                            if discount.set_quantity:
                                if discount.quantity > 0:
                                    ...
                                    messages.error(
                                        request, f"'{discount.code}' has no valid uses left."
                                    )
                            ...
                            messages.error(request, f"'{discount.code}' has expired.")
                            del request.session["discount"]
                            order.delete()
                            return redirect(reverse('view_bag'))
                    ...
                    messages.error(
                        request, f"'{discount.code}' is not an active discount code."
                    )
                    del request.session["discount"]
                    order.delete()
                    return redirect(reverse('view_bag'))
